...
import { DoorOpen, Plus, Thermometer, Droplets, Compass, Sun, Trash2, X, Loader2, Cloud, SunDim, Wind } from "lucide-react";
...
export default function RoomsManager({ rooms }: { rooms: any[] }) {
  ...
  // États locaux pour nos nouveaux composants UI custom
  const [orientations, setOrientations] = useState<string[]>([]);
  const [lightLevel, setLightLevel] = useState("Moyenne"); // Moyenne par défaut
  const [humidityLevel, setHumidityLevel] = useState("Normale"); // Normale par défaut
  ...
  const toggleOrientation = (dir: string) => {
    if (orientations.includes(dir)) {
      ...
      if (orientations.length < 2) {
        ...
        setOrientations([orientations[0], dir]); // Remplace le dernier si on en sélectionne un 3ème
      }
    }
  };
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (result.error) toast.error(result.error);
        ...
        // On réinitialise le formulaire et on le ferme
        setIsAdding(false);
        setOrientations([]);
  ...
  return (
    <section className="space-y-4 mt-8">
      ...
      {/* FORMULAIRE D'AJOUT */}
      {isAdding && (
        <div className="bg-white p-5 sm:p-6 rounded-[2rem] border border-emerald-200/60 shadow-xl shadow-emerald-900/5 animate-in fade-in slide-in-from-top-4">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-bold text-stone-800 text-lg">Nouvel emplacement</h3>
            <Button onClick={() => setIsAdding(false)} variant="ghost" size="icon" className="h-8 w-8 text-stone-400 hover:text-stone-600 bg-stone-50 rounded-full"><X className="w-4 h-4" /></Button>
          </div>
          ...
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <div className="space-y-2.5">
              ...
              <Input name="name" required placeholder="Ex: Salon, Couloir, Véranda..." className="h-12 rounded-2xl bg-[#FDFCF8] border-stone-200 focus:ring-emerald-500 font-medium" />
            </div>

            {/* 2. ORIENTATION (Boutons Multi-choix) */}
            <div className="space-y-2.5">
                    ...
                    <button
                      ...
                      className={`py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all border active:scale-95 ${
                        isSelected 
                          ...
                          : 'border-stone-200 bg-[#FDFCF8] text-stone-500 hover:border-stone-300'
                      }`}
                    >
                      {dir}
                    </button>
            ...
            {/* 3. LUMINOSITÉ (Segmented Control) */}
            <div className="space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-4 pt-2">
              <div className="space-y-2.5">
                ...
                <div className="relative">
                  <Input name="temp_summer" type="number" placeholder="Ex: 25" className="h-12 rounded-2xl bg-[#FDFCF8] border-stone-200 pr-8 font-medium" />
              ...
              <div className="space-y-2.5">
                ...
                <div className="relative">
                  <Input name="temp_winter" type="number" placeholder="Ex: 19" className="h-12 rounded-2xl bg-[#FDFCF8] border-stone-200 pr-8 font-medium" />
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-stone-400 text-sm font-bold">°C</span>
                </div>
              </div>
            </div>
            ...
            <Button type="submit" disabled={isPending} className="w-full h-14 mt-4 rounded-2xl bg-emerald-800 text-white text-lg font-bold hover:bg-emerald-900 transition-all shadow-lg shadow-emerald-900/20 active:scale-95">
              {isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : "Créer l'emplacement"}
            </Button>
          </form>
        </div>
      )}
      ...
      <div className="space-y-3">
        {rooms.length === 0 && !isAdding && (
          <div className="text-center p-8 bg-white/50 border border-stone-200 border-dashed rounded-[2rem]">
            <div className="w-12 h-12 bg-stone-100 rounded-full flex items-center justify-center mx-auto mb-3">
              <DoorOpen className="w-6 h-6 text-stone-400" />
            </div>
            <p className="text-sm text-stone-500 font-medium">Vous n'avez pas encore défini de pièces.<br/>Ajoutez un emplacement pour aider l'IA.</p>
          </div>
        )}
        
        {rooms.map(room => (
          <div key={room.id} className="bg-white p-5 rounded-[1.5rem] border border-stone-100 shadow-sm flex items-center justify-between group">
            <div>
              <h3 className="font-extrabold text-stone-800 text-lg mb-1 flex items-center gap-2">
                {room.name}
              </h3>
              <div className="flex flex-wrap gap-2 mt-2">
                {room.orientation && <span className="text-[10px] font-bold uppercase tracking-wider bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-md flex items-center gap-1"><Compass className="w-3 h-3" /> {room.orientation}</span>}
                {room.light_level && <span className="text-[10px] font-bold uppercase tracking-wider bg-amber-50 text-amber-600 px-2.5 py-1 rounded-md flex items-center gap-1"><Sun className="w-3 h-3" /> {room.light_level}</span>}
                {room.humidity && <span className="text-[10px] font-bold uppercase tracking-wider bg-blue-50 text-blue-600 px-2.5 py-1 rounded-md flex items-center gap-1"><Droplets className="w-3 h-3" /> {room.humidity}</span>}
                {(room.temp_summer || room.temp_winter) && (
                  <span className="text-[10px] font-bold uppercase tracking-wider bg-stone-100 text-stone-600 px-2.5 py-1 rounded-md flex items-center gap-1">
            ...
            <Button onClick={() => handleDelete(room.id)} disabled={isPending} variant="ghost" size="icon" className="text-stone-300 hover:text-rose-500 hover:bg-rose-50 rounded-full h-10 w-10 shrink-0 transition-colors">
